...
class DataHandler(object):
    ...
    def __init__(self):
        ...
        if not self.database_path:
            print('Stats not found.\n'\
                  'Creating stat database in {}...'
                  .format(self.originalpath))
    ...
    #   -create data management screen with db-path-set field
...
class Smash(Frame):
    ...
    def __init__(self, master):
        ...
        '''
        # perhaps make boolean keyword parameter for "dev mode",
        # where player instantiation and screen calls are scripted. 
        # if devmode:
        #     # skip playerselect using preset player identities
        # else:
        #     # don't worry
    ...
    def titlescreen(self):
        ...
        #   -maybe recreate the focus-based 'select-ball' 
        #    on right of option
    ...
    def playerselect(self):
        ...
        self.coinlist = [self.p1coin, self.p2coin,
                         self.p3coin, self.p4coin]
        self.emptylist = [self.p1empty, self.p2empty,
                          self.p3empty, self.p4empty]
        ...
        def nameselectbox(self, togglebutton, namebutton, player):
            ...
            def nameupdate(player):
                ...
                if dupflag == True:
                    ...
                    namebutton.config(text = player.name, 
                                      font = ('Helectiva', 15))
            
            
            def nameregister(event, player):
                ...
                elif name == None or name == '' or name == "''":
                    print('Name = None or Name = \'\' or Name == \"\'\'\"') 
                else:
                    self.datahandler.cur.execute(
            ...
            def nameentry():
                ...
                newnameentry = Entry(self, text = 'NEW NAME ENTRY',
                bd = 0, bg = 'white', fg = 'black', font = ('Arial', 12))
                self.canvas.create_window((xpos + 89, ypos + 228), 
                width = 156, height = 23,
                window = newnameentry, tags = ('p{}'
                                               .format(togglebutton.slot),
                                               'p{}box'
                                               .format(togglebutton.slot)))
                newnameentry.bind('<Return>', 
                                  lambda event: nameregister(event, player))
                savebutton = Button(self, text = 'Done',
                                    font = ('Arial', 9),
                                    highlightthickness = 0,
                                    command = lambda: nameregister(newnameentry))
                self.canvas.create_window((xpos + 150, 
                                           ypos + 208),
                                           width = 37, 
                                           height = 17,
                                           window = savebutton, 
        ...
        # this next block somehow fails if placed 
        # in resourcing block at top
    ...
    def recordingscreen(self):
        ...
        self.recordingbackground = PhotoImage(file = 'recordingscreen.png')
        self.canvas.create_image((400,300), 
                                 image = self.recordingbackground)
        self.statbgs = [PhotoImage(file = 'statbox.png') 
                        for slot in range(4)]
        
        def statframe(player):
            
            x0 = 45; y = 225
            x = x0  + (player.slot - 1) * 175 + 1*(player.slot - 1)
            
            statimage = Label(self, cursor = 'hand1', 
                              bd = 0, highlightthickness = 0,
                              image = self.statbgs[player.slot - 1])
            self.canvas.create_window((x + 90, y + 160), 
                                      window = statimage,
                                      tags = ('p{}'
                                              .format(player.slot),
                                              'p{}bg'
                                              .format(player.slot)))
            namelabel = Label(self, font = ('Helectiva', 15), 
                              cursor = 'hand1', 
                              bd = 0, bg = 'black', fg = '#B2C7DE',
                              highlightthickness = 0, text = player.name)
            self.canvas.create_window((x + 90, y + 322),
                                      window = namelabel,
                                      tags = ('p{}'
                                              .format(player.slot),
                                              'p{}name'
                                              .format(player.slot)))
            statnames = Listbox(self,
                             bg = 'green', fg = 'black',
                             bd = 0, highlightthickness = 0,
                             activestyle = 'dotbox',
                             height = len(self.activeplayers) - 1,
                             font = ('Helectiva', 14,'bold'),
                             cursor = 'hand1')
            self.canvas.create_window((x + 74, 
                                       y + 135+ 
                                       10 * len(self.activeplayers) - 1), 
                                       width = 120, 
                                       # height = 275 or something
                                       window = statnames)
            entryholder = Frame(self, 
                                bd = 0, container = 0,
                                bg = 'cyan', highlightthickness = 0,
                                cursor = 'hand1')
            self.canvas.create_window((x + 150, y + 215), 
                                      height = 150, width = 30,
                                      window = entryholder)
            ...
            def entrycheck(entry):
                ...
                value = entry.get()
                print('\nChecking entry...\n')
                print('Reference Player Name: {}'
                      .format(entry.reference.name))
                print('Entry contents: {}'.format(value))
                
                print('Trying to eval... ')
                
                try:
                    print(eval(value))
                    print('int version: {}'.format(int(value)))
            ...
            rownum = 0
            for user in self.activeplayers:
                if user.slot != player.slot:
                    opponent = user
                    statnames.insert(END, opponent.name)
                    count = StringVar()
                    entry = Entry(entryholder, 
                                  bd = 0, highlightthickness = 1,
                                  font = ('Helectiva', 14, 'bold'),
                                  validate = 'focusout',
                                  # validatecommand = 
                                  # lambda: entrycheck(entry)
                                  )
                    entry.bind('<FocusOut>', 
                               lambda event: entrycheck(entry))
                    entry.reference = opponent
                    entry.pack()
        ...
        for slot in range(len(self.playerlist)):
            if self.playerlist[slot] != None:
                print('Player found in slot {}'.format(slot + 1))
                self.activeplayers.append(self.playerlist[slot])        
